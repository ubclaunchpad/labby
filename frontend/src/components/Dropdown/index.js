...
function Dropdown({ question }) {
  ...
  useEffect(() => {
    ...
    const draft = draftList.find((draft) => draft.fk_question_id === question.question_id);
    if (draft) {
      const value = optionList.find((option) => option.answer_id === draft.answer);
  ...
  return (
    <div className="GlobalCustomerQuestionContainer">
      ...
      <select className="select" defaultValue={selectedValue} onChange={handleChange}>
